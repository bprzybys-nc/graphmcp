...
from utils.source_type_classifier import SourceType, SourceTypeClassifier
...
class AgenticFileProcessor:
    ...
    def __init__(
        ...
        self.logger = get_logger(
            workflow_id=f"agentic_processor_{repo_owner}_{repo_name}", config=config
        )
    ...
    def _build_agent_prompt(
        ...
        database_name = getattr(
            self.contextual_rules_engine, "database_name", "unknown"
        )
    ...
    async def process_files(
        ...
        for source_type, files in categorized_files.items():
            ...
            for i in range(0, len(files), batch_size):
                ...
                self.logger.log_info(
                    f"Built prompt for batch (category: {source_type.value}), invoking agent..."
                )
    ...
    try:
        ...
        if USE_AGENTIC_PROCESSOR:
            ...
            for file_info in discovered_files:
                ...
                # Classify file
                classification = source_classifier.classify_file(
                    file_path, file_content
                )

                # Process with contextual rules
                processing_result = (
                    await contextual_rules_engine.process_file_with_contextual_rules(
                        file_path,
                        file_content,
                        classification,
                        database_name,
...
async def log_pattern_discovery_visual(
    ...
    try:
        ...
        if files_by_type:
            ...
            logger.log_tree(
                f"File Type Distribution: {repo_owner}/{repo_name}", distribution_tree
            )
...
def extract_high_confidence_patterns(
    discovery_result: Dict[str, Any], confidence_threshold: float = 0.8
) -> List[Dict[str, Any]]:
    """
