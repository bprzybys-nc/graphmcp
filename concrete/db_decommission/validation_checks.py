...
async def perform_database_reference_check(
    ...
    try:
        ...
        for file_info in files:
            ...
            direct_references = len(
                re.findall(
                    rf"\b{re.escape(database_name)}\b", file_content, re.IGNORECASE
                )
            )
    ...
    try:
        ...
        if total_confidence_files == 0:
            ...
            if confidence >= 80:
                status = ValidationResult.PASSED.value
                description = (
                    f"High quality pattern discovery: {confidence}% confidence"
                )
            elif confidence >= 60:
                status = ValidationResult.WARNING.value
                description = (
                    f"Medium quality pattern discovery: {confidence}% confidence"
                )
            else:
                status = ValidationResult.FAILED.value
    ...
    try:
        ...
        if total_files == 0:
            ...
            if critical_ratio < 0.2:  # Less than 20% critical files
                ...
                status = ValidationResult.PASSED.value
                description = (
                    f"Low impact: {total_critical_files}/{total_files} critical files"
                )
                risk_level = "LOW"
                ...
                status = ValidationResult.FAILED.value
                description = (
                    f"High impact: {total_critical_files}/{total_files} critical files"
                )
                risk_level = "HIGH"
...
def _generate_integrity_recommendations(
    critical_files: List[Dict[str, Any]], risk_level: str
) -> List[str]:
    """
...
def generate_recommendations(
    ...
    failed_checks = [
        ...
        if check.get("status") == ValidationResult.FAILED.value
    ]
    warning_checks = [
        check
        for check in qa_checks
        if check.get("status") == ValidationResult.WARNING.value
    ]
