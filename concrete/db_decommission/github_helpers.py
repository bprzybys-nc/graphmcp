...
"""
    ...
    for source_type, file_paths in files_by_type.items():
        modified_count = len(
            [f for f in modified_files if f.get("source_type") == source_type]
        )
        if modified_count > 0:
...
"""
    for file_result in modified_files:
        pr_body += (
            f"- `{file_result['path']}` ({file_result['changes_made']} changes)\n"
        )
...
async def create_fork_and_branch(
    github_client: Any, repo_owner: str, repo_name: str, database_name: str, logger: Any
) -> Dict[str, Any]:
    """
    ...
    branch_result = await github_client.create_branch(
        fork_owner, repo_name, branch_name
    )
...
async def commit_file_changes(
    ...
    for file_result in modified_files:
        ...
        if update_result.get("success", False):
            ...
            logger.log_warning(
                f"Failed to commit: {file_path} - {update_result.get('error', 'Unknown error')}"
            )

    return {"files_committed": files_committed, "commit_messages": commit_messages}
...
async def create_pull_request(
    ...
    pr_body = await _create_pr_body(
        database_name, modified_files, refactoring_result, files_committed
    )

    logger.log_info(f"📝 Creating pull request: {pr_title}")

    pr_result = await github_client.create_pull_request(
        repo_owner, repo_name, pr_title, f"{fork_owner}:{branch_name}", "main", pr_body
    )

    if not pr_result.get("success", False):
        raise RuntimeError(
            f"Failed to create pull request: {pr_result.get('error', 'Unknown error')}"
        )

    pr_number = pr_result.get("number", "Unknown")
    pr_url = (
        pr_result.get("url")
        or pr_result.get("html_url")
        or f"https://github.com/{repo_owner}/{repo_name}/pulls"
    )
