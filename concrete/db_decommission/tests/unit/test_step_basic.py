...
def test_step_auto_basic():
    ...
    assert step.step_type == StepType.CUSTOM, (
        f"Expected CUSTOM step type, got {step.step_type}"
    )
    assert step.parameters["test_param"] == "test_value", (
        "Expected test_param 'test_value'"
    )
    assert step.timeout_seconds == 45, (
        f"Expected timeout 45, got {step.timeout_seconds}"
    )
...
def test_database_workflow_builder():
    ...
    assert result is builder, (
        "with_slack_channel should return self for method chaining"
    )
    ...
    assert repo_params["target_repos"] == [
        "https://github.com/test/repo1",
        "https://github.com/test/repo2",
    ]
...
def test_create_db_decommission_workflow():
    ...
    assert isinstance(builder, DatabaseDecommissionWorkflowBuilder), (
        "Should return DatabaseDecommissionWorkflowBuilder"
    )
...
def test_extract_repo_details():
    ...
    assert name == "postgres-sample-dbs", (
        f"Expected fallback name 'postgres-sample-dbs', got {name}"
    )
