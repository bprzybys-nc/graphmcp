...
from .repository_processors import initialize_github_client
...
async def quality_assurance_step(
    ...
    try:
        ...
        if not discovery_result or "files" not in discovery_result:
            logger.log_warning(
                "No discovery results found, creating empty structure for QA validation"
            )
            discovery_result = {
                "files": [],
                "files_by_type": {},
        ...
        logger.log_info(
            f"QA validation processing {total_files} files from discovery results"
        )
        ...
        rule_check = await perform_rule_compliance_check(
            discovery_result, database_name
        )
        logger.log_info(
        ...
        integrity_check = await perform_service_integrity_check(
            discovery_result, database_name
        )
        logger.log_info(
        ...
        result = {
            ...
            "all_checks_passed": all(
                check["status"] == ValidationResult.PASSED.value for check in qa_checks
            ),
            "quality_score": quality_score,
            "recommendations": recommendations,
            "success": True,
            ...
            qa_summary_data.append(
                {
                    ...
                    "status": (
                        "passed"
                        if check["status"] == ValidationResult.PASSED.value
                        else "failed"
                    ),
...
async def apply_refactoring_step(
    ...
    try:
        ...
        source_dir = discovery_result.get(
            "extraction_directory", f"tests/tmp/pattern_match/{database_name}"
        )
        ...
        logger.log_info(
            f"Processing files in {source_dir} with FileDecommissionProcessor"
        )
        ...
        total_files_modified = len(
            [
                ...
                if strategies_applied.get(f)
                in ["infrastructure", "configuration", "code"]
            ]
        )
        ...
        for file_path in processed_files:
            ...
            output_dir = Path(
                processing_result.get(
                    "output_directory", f"{source_dir}_decommissioned"
                )
            )
            relative_path = (
            ...
            if modified_file.exists():
                ...
                changes_made = (
                    1 if strategy in ["infrastructure", "configuration", "code"] else 0
                )

                # Map back to original repository path (remove extraction directory prefix)
                original_path = (
                    str(relative_path) if source_dir in file_path else file_path
                )
                ...
                for discovered_file in discovery_result.get("matched_files", []):
                    if discovered_file.get("extracted_path") == file_path:
                        original_path = discovered_file.get(
                ...
                if changes_made > 0:
                    logger.log_info(
                        f"Modified {file_path} ({strategy}): {changes_made} changes"
                    )
            ...
            [
                {
                    ...
                    "modified_files": len(
                        [
                            ...
                            if r.get("source_type") == strategy
                            and r.get("changes_made", 0) > 0
                        ]
                    ),
                }
...
async def create_github_pr_step(
    ...
    try:
        ...
        if not refactoring_files:
            ...
            return {"success": False, "message": "No refactoring results to commit"}
...
async def workflow_summary_step(
    ...
    try:
        ...
        success_rate = (
            100.0 if qa_result.get("all_checks_passed", False) else quality_score
        )
        ...
        decommissioning_summary = DecommissioningSummary(
            ...
            total_changes=sum(
                f.get("changes_made", 0)
                for f in refactoring_result.get("refactoring_results", [])
            ),
            rules_applied=list(refactoring_result.get("files_by_type", {}).keys()),
            execution_time_seconds=time.time()
            - context.get_shared_value("workflow_start_time", start_time),
        ...
        logger.log_table(
            ...
            [
                {
                    "metric": "Repositories Processed",
                    "value": str(total_repositories),
                    "status": "✅",
                },
                {
                    "metric": "Files Discovered",
                    "value": str(repository_result.get("total_files_processed", 0)),
                    "status": "✅",
                },
                {
                    "metric": "Files Processed",
                    "value": str(total_files_processed),
                    "status": "✅",
                },
                {
                    "metric": "Files Modified",
                    "value": str(total_files_modified),
                    "status": "✅",
                },
                {
                ...
                },
                {
                    ...
                    "value": f"{decommissioning_summary.execution_time_seconds:.1f}s",
                    "status": "✅",
                },
            ],
        )
