...
class ErrorRecoveryStrategy:
    ...
    async def execute_with_recovery(self, operation: Callable, *args, **kwargs) -> Any:
...
class ErrorHandler:
    ...
    def _setup_default_strategies(self):
        ...
        self.recovery_strategies = {
            ...
            ErrorCategory.EXTERNAL_SERVICE: ErrorRecoveryStrategy(
                max_retries=2, backoff_factor=3.0
            ),
            ErrorCategory.RESOURCE: ErrorRecoveryStrategy(
                max_retries=1, backoff_factor=1.0
            ),
            ErrorCategory.AUTHENTICATION: ErrorRecoveryStrategy(
                max_retries=1, backoff_factor=1.0
            ),
        }
    ...
    async def handle_error(
        ...
        error_context = ErrorContext(
            ...
            function_name=(
                traceback.extract_tb(exception.__traceback__)[-1].name
                if exception.__traceback__
                else "unknown"
            ),
            module_name=(
                traceback.extract_tb(exception.__traceback__)[-1].filename
                if exception.__traceback__
                else "unknown"
            ),
            metadata=context or {},
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        for error in self.error_history:
            severity_count[error.severity.value] = (
                severity_count.get(error.severity.value, 0) + 1
            )
            category_count[error.category.value] = (
                category_count.get(error.category.value, 0) + 1
            )
