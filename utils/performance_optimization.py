"""
Performance Optimization Module for GraphMCP Workflows.

This module provides caching mechanisms, async optimizations, connection pooling,
and other performance enhancements for production-scale workflow execution.
Extracted from concrete implementation for reuse across different workflows.
"""
...
class AsyncCache:
    ...
    async def set(
        ...
        if (
            self.strategy in [CacheStrategy.MEMORY, CacheStrategy.HYBRID]
            or promote_to_memory
        ):
...
class ConnectionPool:
    ...
    def __init__(
        self, max_connections: int = 100, timeout: float = 30.0, retry_attempts: int = 3
    ):
        ...
        self._http_session = aiohttp.ClientSession(
            connector=self._http_connector, timeout=aiohttp.ClientTimeout(total=timeout)
        )
...
class ParallelProcessor:
    ...
    async def process_batch(
        self, func: Callable, items: List[Any], batch_size: int = 10, **kwargs
    ) -> List[Any]:
...
class PerformanceManager:
    ...
    async def optimize_repository_processing(
        self, repositories: List[str], processor_func: Callable
    ) -> List[Any]:
    ...
    async def cached_api_call(
        self, cache_key: str, api_func: Callable, *args, ttl: int = 1800, **kwargs
    ) -> Any:
