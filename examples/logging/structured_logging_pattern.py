...
def sunburst_logging_example():
    ...
    logger_instance.log_sunburst(
        processing_labels, processing_parents, processing_values, "Processing Results"
    )
...
def progress_logging_example():
    ...
    for batch in range(0, total_items, 10):
        ...
        logger_instance.log_info(
            f"- Estimated time remaining: {(total_items - processed_items) * 0.1:.1f}s"
        )

    logger_instance.log_info(
        f"✅ **Batch processing completed**: {processed_items} items processed"
    )
...
def context_logging_example():
    ...
    # Log with context
    logger_instance.log_info("🔄 **Workflow started**", context=workflow_context)
    ...
    logger_instance.log_info("✅ **Step completed successfully**", context=step_context)
    ...
    logger_instance.log_error(
        "❌ **Step failed with validation error**", context=error_context
    )
...
def custom_formatter_example():
    ...
    logger_instance.info(
        ...
        extra={"workflow_id": "custom_workflow", "step_name": "custom_step"},
    )
...
async def comprehensive_logging_example():
    ...
    try:
        ...
        for i, repo in enumerate(repos):
            logger_instance.log_info(
                f"📁 **Processing repository {i + 1}/{len(repos)}: {repo}**"
            )
            await asyncio.sleep(0.1)
