...
async def example_validation_step(context: WorkflowContext) -> Dict[str, Any]:
    ...
    try:
        ...
        missing_params = [param for param in required_params if param not in context.parameters]
...
async def example_processing_step(context: WorkflowContext) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, repo in enumerate(target_repos):
            logger_instance.log_info(f"Processing repository {i+1}/{len(target_repos)}: {repo}")
        ...
        result = {
            ...
            "total_references": sum(repo["references_found"] for repo in processed_repos)
        }
        
        # Log summary table
        table_data = [
            ["Repository", "Files", "References"],
            *[[repo["repo_url"], repo["files_found"], repo["references_found"]] for repo in processed_repos]
        ]
...
async def example_notification_step(context: WorkflowContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not processing_result or not processing_result.get("success"):
            ...
            logger_instance.log_step_end("notification", {"error": error_msg}, success=False)
            return {"success": False, "error": error_msg}
...
async def create_example_workflow(config_path: str) -> Any:
    ...
    workflow = (WorkflowBuilder("example_workflow", config_path)
        ...
        .step_auto("notification", "Send completion notification", example_notification_step)
        .build())
...
async def create_advanced_workflow(config_path: str) -> Any:
    ...
    workflow = (WorkflowBuilder("advanced_workflow", config_path)
        .with_config(
...
async def parallel_workflow_example():
    ...
    workflow = (WorkflowBuilder("parallel_workflow", config_path)
        .with_config(
            max_parallel_steps=3,  # Allow 3 parallel steps
            default_timeout=300
        )
        .step_auto("validation", "Validate parameters", example_validation_step)
...
async def error_handling_workflow_example():
    ...
    workflow = (WorkflowBuilder("error_handling_workflow", config_path)
        .with_config(
