...
class TestFilesystemWorkflowIntegration:
    ...
    async def test_filesystem_client_initialization(self, mock_config_path):
        ...
        assert (
            str(client.config_path) == mock_config_path
    ...
    async def test_write_file_workflow_pattern(self, mock_config_path):
        ...
        mock_response = {
            "content": [
                {"type": "text", "text": "Successfully wrote to test_workflow.txt"}
            ]
        }

        with patch.object(
            client, "call_tool_with_retry", new_callable=AsyncMock
        ) as mock_call:
            mock_call.return_value = mock_response

            result = await client.write_file(
                "test_workflow.txt", "Workflow validation content"
            )

            assert result is True
            mock_call.assert_called_once_with(
                "write_file",
                {"path": "test_workflow.txt", "content": "Workflow validation content"},
            )
    ...
    async def test_read_file_workflow_pattern(self, mock_config_path):
        ...
        mock_response = {
            "content": [
                {"type": "text", "text": "Windsurf rules validated successfully"}
            ]
        }
        ...
        with patch.object(
            ...
            mock_call.assert_called_once_with(
                "read_file", {"path": ".windsurf/rules.md"}
            )
    ...
    async def test_list_directory_workflow_pattern(self, mock_config_path):
        ...
        with patch.object(
            ...
            mock_call.assert_called_once_with("list_directory", {"path": "."})
    ...
    async def test_health_check_workflow_integration(self, mock_config_path):
        ...
        with patch.object(
            client, "list_available_tools", new_callable=AsyncMock
        ) as mock_tools:
    ...
    async def test_current_directory_pattern_from_prototype(self, mock_config_path):
        ...
        write_response = {
            "content": [
                {"type": "text", "text": "Successfully wrote to test_current.txt"}
            ]
        }

        # Mock read from current directory
        read_response = {"content": [{"type": "text", "text": test_content}]}
...
class TestFilesystemDataModels:
    ...
    def test_filesystem_scan_result_empty(self):
        ...
        result = FilesystemScanResult(
            base_path=".", pattern="*.nonexistent", files_found=[], matches=[]
        )
