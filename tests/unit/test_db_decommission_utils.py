...
class TestCreateDbDecommissionWorkflow:
    ...
    def test_create_workflow_with_defaults(self, mock_workflow_builder):
        ...
        assert (
            mock_builder_instance.custom_step.call_count == 6
    ...
    def test_create_workflow_repo_extraction(
        self, mock_extract_repo, mock_workflow_builder
    ):
        ...
        # Verify
        mock_extract_repo.assert_called_once_with(
        ...
        step_calls = mock_builder_instance.custom_step.call_args_list
        repo_specific_steps = [
            call for call in step_calls if "repo_owner" in call[1]["parameters"]
        ]
...
class TestRunDecommission:
    ...
    async def test_run_decommission_success(
        self,
        mock_file,
        mock_logging_config,
        mock_get_logger,
    ...
    @patch("concrete.db_decommission.utils.LoggingConfig")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_decommission_workflow_error(
        self,
        mock_file,
        mock_logging_config,
        mock_get_logger,
        ...
        mock_logger.log_info.assert_called_with(
            "Stopping workflow and cleaning up MCP servers..."
        )
...
class TestUtilityFunctions:
    ...
    def test_validate_workflow_parameters_invalid_repos(self):
        ...
        result = validate_workflow_parameters(
            database_name="test_db", target_repos=[], slack_channel="C123456789"
        )
        ...
        assert result["valid"] is False
        assert any(
            "Invalid repository URL format" in error for error in result["errors"]
        )
