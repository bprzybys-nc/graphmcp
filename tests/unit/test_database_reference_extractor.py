...
def helper_function():
    ...
    @pytest.mark.parametrize(
        "target_repo_pack_path,database_name",
    ...
    async def test_real_extraction(self, target_repo_pack_path, database_name):
        ...
        result = await extractor.extract_references(
            database_name=database_name, target_repo_pack_path=target_repo_pack_path
        )
        ...
        text_files = [
            ...
            if f.is_file()
            and f.suffix in [".yaml", ".yml", ".py", ".sql", ".md", ".txt"]
        ]
        ...
        assert found_reference, (
            f"No references to {database_name} found in extracted files"
        )
