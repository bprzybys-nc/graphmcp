...
class TestStepMethod:
    ...
    def test_step_method_chaining(self, mock_config_path):
        ...
        result_builder = (
            builder.step("step1", "First Step", test_delegate_function)
            .step(
                "step2", "Second Step", lambda_compatible_function, depends_on=["step1"]
            )
            .step(
                "step3", "Third Step", workflow_summary_function, depends_on=["step2"]
            )
        )
    ...
    def test_mixed_usage_workflow(self, mock_config_path):
        ...
        workflow = (
            builder.step(
                "legacy_step",
                "Legacy Step",
                lambda context, step, **params: test_delegate_function(
                    context, step, **params
                ),
            )
            ...
            .step(
                "another_legacy",
                "Another Legacy",
                lambda context, step, **params: workflow_summary_function(
                    context, step, **params
                ),
            )
...
class TestStepMethodExecution:
    ...
    async def test_step_and_custom_step_execution_equivalent(self, mock_config_path):
        ...
        workflow2 = builder2.step(
            "test_step", "Test Step", test_delegate_function, parameters={"value": 100}
        ).build()
    ...
    async def test_multi_step_workflow_with_step_method(self, mock_config_path):
        ...
        workflow = (
            builder.step("step1", "First Step", step1_func)
