...
class TestAgenticFileProcessor:
    ...
    def setup_method(self):
        ...
        # Mock logger setup
        with patch(
            "concrete.db_decommission.pattern_discovery.get_logger"
        ) as mock_get_logger:
    ...
    def test_initialization(self):
        ...
        assert (
            self.processor.contextual_rules_engine == self.mock_contextual_rules_engine
        )
    ...
    def test_build_agent_prompt(self):
        ...
        batch = [
            ...
            {"file_path": "test/file2.py", "file_content": "import psycopg2"},
        ]
    ...
    async def test_invoke_agent_on_batch_success(self, mock_openai):
        ...
        mock_response.choices[0].message.content = json.dumps(
            {
                "test/file1.py": {"modified_content": "# DATABASE_URL removed"},
                "test/file2.py": {"modified_content": "# psycopg2 removed"},
            }
        )
        ...
        self.mock_contextual_rules_engine._update_file_content.return_value = (
            AsyncMock()
        )
    ...
    async def test_process_files_success(self):
        ...
        with patch.object(
            self.processor, "_invoke_agent_on_batch", return_value=[mock_result]
        ):
            # Test data
            files_to_process = [
                {
    ...
    async def test_process_files_missing_content(self):
        ...
        files_to_process = [
            ...
            {"file_path": "test/file2.py", "file_content": None},  # Missing content
        ]
...
class TestPatternDiscoveryFunctions:
    ...
    async def test_process_discovered_files_with_rules_agentic(self):
        ...
        with patch(
            "concrete.db_decommission.pattern_discovery.AgenticFileProcessor"
    ...
    async def test_process_discovered_files_with_rules_sequential(self):
        ...
        mock_contextual_rules_engine.process_file_with_contextual_rules.return_value = (
        ...
        discovery_result = {
            "files": [{"path": "test/file1.py", "content": "DATABASE_URL = 'test'"}]
        }

        # Patch to use sequential processor
        with patch(
            "concrete.db_decommission.pattern_discovery.USE_AGENTIC_PROCESSOR", False
        ):
            # Execute
            result = await process_discovered_files_with_rules(
                mock_context,
    ...
    async def test_log_pattern_discovery_visual_error(self):
        ...
        discovery_result = {"files_by_type": {"python": [{"path": "file1.py"}]}}
