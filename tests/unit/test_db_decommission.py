...
class TestMainModule:
    ...
    async def test_main_function_success(
        self, mock_logging, mock_format_summary, mock_run_decommission
    ):
...
class TestWorkflowStepFunctions:
    ...
    async def test_validate_environment_step_callable(
        ...
        result = await validate_environment_step(
            mock_context, mock_step, database_name="test_db"
        )
