...
class TestPreviewMCPClient:
    ...
    async def test_list_available_tools_fallback(self):
        ...
        with patch.object(
            client, "_send_mcp_request", side_effect=Exception("Connection failed")
        ):
    ...
    async def test_health_check_success(self):
        ...
        with patch.object(
            client, "list_available_tools", return_value=["create_workflow"]
        ):
    ...
    async def test_health_check_failure(self):
        ...
        with patch.object(
            client, "list_available_tools", side_effect=Exception("Connection failed")
        ):
            result = await client.health_check()
    ...
    async def test_create_workflow(self):
        ...
        with patch.object(
            client, "call_tool_with_retry", return_value=expected_result
        ) as mock_call:
            result = await client.create_workflow(
                "test_workflow", [{"name": "step1"}, {"name": "step2"}], "user123"
            )
    ...
    async def test_execute_workflow(self):
        ...
        mock_call.assert_called_once_with(
            "execute_workflow", {"workflow_id": "test-workflow-id"}
        )
        assert result == expected_result
    ...
    async def test_run_workflow_end_to_end(self):
        ...
        with (
            patch.object(
                client, "create_workflow", return_value=create_result
            ) as mock_create,
            patch.object(
                client, "execute_workflow", return_value=execute_result
            ) as mock_execute,
        ):
            result = await client.run_workflow_end_to_end(
                "test_workflow", [{"name": "step1"}], "user123"
            )
