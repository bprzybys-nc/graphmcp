...
class TestGitHubMCPClientUnitTests:
    ...
    def mock_mcp_response_fork_success(self):
        ...
        fork_data = {
            ...
            "owner": {"login": "testuser", "id": 12345, "type": "User"},
        ...
        return {"content": [{"type": "text", "text": json.dumps(fork_data)}]}
    ...
    def mock_mcp_response_branch_success(self):
        ...
        # Return in MCP response format
        return {"content": [{"type": "text", "text": json.dumps(branch_data)}]}

    @pytest.mark.asyncio
    async def test_fork_repository_success(
        self, github_client, mock_mcp_response_fork_success
    ):
        """Test successful repository forking."""
        with patch.object(
            github_client, "call_tool_with_retry", new_callable=AsyncMock
        ) as mock_call:
            mock_call.return_value = mock_mcp_response_fork_success

            result = await github_client.fork_repository(
                "bprzybys-nc", "postgres-sample-dbs"
            )

            # Verify the tool was called with correct parameters
            mock_call.assert_called_once_with(
                "fork_repository",
                {"owner": "bprzybys-nc", "repo": "postgres-sample-dbs"},
            )
            ...
            assert (
                result["html_url"] == "https://github.com/testuser/postgres-sample-dbs"
            )
    ...
    async def test_fork_repository_with_organization(self, github_client):
        ...
        org_fork_data = {
            ...
            "owner": {"login": "test-org", "id": 54321, "type": "Organization"},
        ...
        mock_org_response = {
            "content": [{"type": "text", "text": json.dumps(org_fork_data)}]
        }
        ...
        with patch.object(
            ...
            result = await github_client.fork_repository(
                "bprzybys-nc", "postgres-sample-dbs", organization="test-org"
            )
    ...
    async def test_create_branch_success(
        self, github_client, mock_mcp_response_branch_success
    ):
        ...
        with patch.object(
            ...
            assert result["success"] is True
            assert (
                result["ref"] == "refs/heads/decommission-example_database-1642678800"
            )
    ...
    @pytest.mark.asyncio
    async def test_create_branch_with_from_branch(
        self, github_client, mock_mcp_response_branch_success
    ):
    ...
    async def test_create_branch_error_handling(self, github_client):
        ...
        with patch.object(
            ...
            result = await github_client.create_branch(
                "testuser", "postgres-sample-dbs", "existing-branch"
            )
    ...
    async def test_create_pull_request_success(self, github_client):
        ...
        with patch.object(
            ...
            assert (
                result["url"]
                == "https://github.com/testuser/postgres-sample-dbs/pull/42"
            )
...
class TestWorkflowCreation:
    ...
    def test_create_workflow_with_custom_params(self):
        ...
        process_step = next(
            (step for step in workflow.steps if step.id == "process_repositories"), None
        )
...
class TestGitHubClientIntegration:
    ...
    async def test_fork_and_branch_workflow_integration(self, mock_config_path):
        ...
        fork_response = {"content": [{"type": "text", "text": json.dumps(fork_data)}]}
        ...
        with patch.object(
            ...
            branch_result = await github_client.create_branch(
                fork_result["owner"]["login"], fork_result["name"], "test-branch"
            )
