...
class RepoQuickScanTemplate:
    ...
    @staticmethod
    def apply(
        builder: WorkflowBuilder, step_prefix: str, params: dict
    ) -> WorkflowBuilder:
        repo_url = params["repo_url"]

        return (
            builder.repomix_pack_repo(f"{step_prefix}_pack", repo_url)
            .github_analyze_repo(f"{step_prefix}_analyze", repo_url)
            .step(
                f"{step_prefix}_security",
                "Security Scan",
                lambda context, step, **params: security_scan_function(
                    context, step, **params
                ),
                depends_on=[f"{step_prefix}_pack"],
            )
        )
...
class TestWorkflowBuilderExtensions:
    ...
    def test_custom_step_creation(self, mock_config_path):
        ...
        builder.step(
            ...
            lambda context, step, **params: custom_function_helper(
                context, step, **params
            ),
    ...
    def test_workflow_configuration(self, mock_config_path):
        ...
        workflow = (
            builder.repomix_pack_repo("pack", "https://github.com/test/repo")
            .with_config(
    ...
    def test_prompt_template_rendering(self):
        ...
        template = PromptTemplate(
            "Repo: {{ repo_name }}, Files: {{ file_count }}, Status: {{ status }}"
        )
    ...
    def test_prompt_template_nested_values(self):
        ...
        template = PromptTemplate(
            "Issues: {{ security_report.issues }}, Severity: {{ security_report.max_severity }}"
        )

        context = {"security_report": {"issues": 3, "max_severity": "high"}}
        rendered = template.render(context)
...
class TestMockClientFunctionality:
    ...
    async def test_mock_github_client(self, mock_config_path):
        ...
        result = await client.call_tool_with_retry(
            "analyze_repo_structure", {"repo_url": "https://github.com/test/repo"}
        )
    ...
    async def test_mock_slack_client(self, mock_config_path):
        ...
        result = await client.call_tool_with_retry(
            "post_message", {"channel": "C01234567", "text": "Test message"}
        )
    ...
    async def test_mock_repomix_client(self, mock_config_path):
        ...
        result = await client.call_tool_with_retry(
            "pack_remote_repository", {"url": "https://github.com/test/repo"}
        )
        ...
        step_metadata = [
            {
                ...
                "parameters": {
                    k: v for k, v in step.parameters.items() if not callable(v)
                },
            }
    ...
    async def test_multi_step_workflow_execution(self, mock_config_path):
        ...
        workflow = (
            builder.step(
                "step1",
                "First Step",
                lambda context, step, **params: step1_func(context, step, **params),
            )
            .step(
                "step2",
                "Second Step",
                lambda context, step, **params: step2_func(context, step, **params),
                depends_on=["step1"],
            )
            .build()
        )
