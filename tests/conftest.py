...
def mock_config_path(tmp_path_factory):
    ...
    config = {
        "mcpServers": {
            ...
            "ovr_slack": {"command": "echo", "args": ["mock-slack-server"], "env": {}},
            "ovr_repomix": {
                "command": "echo",
...
def real_config_path(tmp_path_factory):
    ...
    github_config = {
        "command": "npx",
        "args": common_args_npx_y
        + ["@modelcontextprotocol/server-github@2025.4.8"],  # Explicit version
        "env": {
    ...
    filesystem_config = {
        ...
        + [
            ...
            str(tmp_path),
        ],  # Added tmp_path as allowed dir
        "env": {"ALLOWED_EXTENSIONS": ".py,.js,.ts,.yaml,.yml,.md,.txt,.json"},
    }
    ...
    for client_class in client_classes:
        ...
        try:
            ...
            if not await client.health_check():
                pytest.skip(
                    f"MCP client {client_class.SERVER_NAME} is not healthy. Skipping E2E tests."
                )
            logger.info(f"MCP client {client_class.SERVER_NAME} is healthy.")
        except Exception as e:
            pytest.skip(
                f"Failed to initialize or health check MCP client {client_class.SERVER_NAME}: {e}. Skipping E2E tests."
            )
        finally:
            if client:
                await client.close()
        ...
        if fork_data.get("created_branch") or fork_data.get("created_fork"):
            ...
            if fork_data.get("created_fork") and fork_data.get("fork_owner"):
                try:
                    ...
                    logger.info(
                        f"✅ Deleted fork: {fork_data['fork_owner']}/{fork_data['fork_name']}"
                    )
                except subprocess.CalledProcessError as e:
