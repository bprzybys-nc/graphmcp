...
@pytest.mark.skipif(
    psutil is None, reason="psutil library not found, skipping resource tests"
)
class TestResourceManagement:
    ...
    async def test_memory_usage_under_load(self, mock_config_path):
        ...
        for i in range(10):
            workflow = (
                WorkflowBuilder(f"load-test-{i}", mock_config_path)
                .step(
                    "step1",
                    "Test Step",
                    lambda ctx, step, **params: {"result": f"test-{i}"},
                )
                .step(
                    "step2",
                    "Test Step 2",
                    lambda ctx, step, **params: {"result": f"test2-{i}"},
                )
                .build()
            )
    ...
    async def test_session_cleanup_mock(self, mock_config_path):
        ...
        with patch(
            "graphmcp.clients.GitHubMCPClient", autospec=True
        ) as MockGitHubClient:
            mock_instance = MockGitHubClient.return_value
            mock_instance.analyze_repo_structure = AsyncMock(
                return_value={"status": "ok"}
            )

            workflow = (
                WorkflowBuilder("session-test", mock_config_path)
            ...
            mock_instance.analyze_repo_structure.assert_called_once_with(
