...
from clients import RepomixMCPClient, GitHubMCPClient, FilesystemMCPClient
...
async def timeout_wrapper(coro, timeout_seconds=30):
    ...
    try:
        ...
        pytest.skip(
            f"Operation timed out after {timeout_seconds}s - MCP server likely unavailable"
        )
    except Exception as e:
        error_str = str(e).lower()
        if any(
            keyword in error_str
            for keyword in ["mcp server", "connection", "timeout", "no response"]
        ):
...
class TestRealIntegration:
    ...
    async def test_real_github_integration(self, session_client_health_check):
        ...
        async def execute_github_workflow():
            workflow = (
                WorkflowBuilder("e2e-github", "clients/mcp_config.json")
                .github_analyze_repo(
                    "analyze", "https://github.com/microsoft/typescript"
                )
                .step(
                    "validate",
                    "Validate Results",
                    lambda context, step, **params: validate_github_results(
                        context, step, **params
                    ),
                    depends_on=["analyze"],
                )
    ...
    async def test_real_slack_integration(self, session_client_health_check):
        ...
        async def execute_slack_workflow():
            workflow = (
                WorkflowBuilder("e2e-slack", "clients/mcp_config.json")
                .slack_post("notify", test_channel, "🧪 E2E test message from GraphMCP")
                .step(
                    "verify",
                    "Verify Slack Post",
                    lambda context, step, **params: verify_slack_post(
                        context, step, **params
                    ),
                    depends_on=["notify"],
                )
    ...
    @pytest.mark.asyncio
    async def test_repomix_analyze_codebase_structure(
        self, session_client_health_check
    ):
        ...
        async def test_repomix_analysis():
            ...
            try:
                ...
                print(
                    f"✅ Repomix analyzed {files_count} files from {result['repository_url']}"
                )
                return result
    ...
    async def test_filesystem_operations(self):
        ...
        async def test_filesystem_ops():
            ...
            try:
                ...
                assert str(test_file.name) in listed_files, (
                    "Test file should be in current directory"
                )
    ...
    async def test_github_repo_operations(self, session_client_health_check):
        ...
        async def test_github_ops():
            ...
            try:
                # Test get_repository
                repo_info = await github_client.get_repository(
                    "microsoft", "typescript"
                )
                assert repo_info["name"] == "typescript"
                assert repo_info["owner"]["login"] == "microsoft"
                print(
                    f"✅ Successfully retrieved GitHub repository info for {repo_info['full_name']}"
                )
                ...
                assert (
                    "TypeScript" in file_content_result
                    or "typescript" in file_content_result.lower()
                )
                print(
                    "✅ Successfully retrieved README.md content for microsoft/typescript"
                )
    ...
    async def test_github_fork_and_branch_operations(
        self, github_test_fork, session_client_health_check
    ):
        ...
        async def test_fork_operations():
            ...
            try:
                ...
                fork_result = await github_client.fork_repository(
                    fork_data["source_owner"], fork_data["source_name"]
                )
                ...
                print(
                    f"✅ Successfully forked repository to {fork_data['fork_owner']}/{fork_data['fork_name']}"
                )
                ...
                assert (
                    "ref" in create_branch_result
                    and fork_data["test_branch"] in create_branch_result["ref"]
                )
    ...
    async def test_db_decommission_workflow_integration(
        self, github_test_fork, session_client_health_check
    ):
        ...
        async def test_db_workflow():
            ...
            if not fork_data.get("fork_owner") or not fork_data.get("test_branch"):
                pytest.skip(
                    "GitHub fork or test branch not available for db_decommission workflow."
                )
            ...
            try:
                ...
                if "find_db_files" in result.step_results:
                    ...
                    assert (
                        dummy_file_path
                        in result.step_results["find_db_files"]["found_files"]
                    )

                print(
                    f"✅ DB Decommission Workflow completed successfully for {dummy_file_path}"
                )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Skipping multi-client coordination test until all clients are working"
    )
        ...
        async def test_coordination():
            ...
            try:
                ...
                if _github_token and len(_github_token) >= 20:
                    try:
                        ...
                        assert repo_info["name"] == "vscode", (
                            "Failed to get VSCode repo info."
                        )
                ...
                read_content = await asyncio.wait_for(
                    filesystem_client.read_file(str(test_local_file)), timeout=5.0
                )
                assert read_content == initial_content, (
                    "Local file content changed unexpectedly."
                )
                ...
                if github_client:
                    cleanup_tasks.append(
                        asyncio.wait_for(github_client.close(), timeout=5.0)
                    )
                if filesystem_client:
                    cleanup_tasks.append(
                        asyncio.wait_for(filesystem_client.close(), timeout=5.0)
                    )
                if repomix_client:
                    cleanup_tasks.append(
                        asyncio.wait_for(repomix_client.close(), timeout=5.0)
                    )
