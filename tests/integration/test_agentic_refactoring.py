    ...
    async def test_pr_creation_step(self, test_data_generator):
            ...
            refactoring_results.append(
                {
                    ...
                    "source_type": (
                        "python" if file_data["file_path"].endswith(".py") else "sql"
                    ),
                }
            )
        ...
        mock_context.get_shared_value.return_value = {
            ...
            "files_by_type": {
                "python": [
                    f["path"]
                    for f in refactoring_results
                    if f["source_type"] == "python"
                ],
                "sql": [
                    f["path"] for f in refactoring_results if f["source_type"] == "sql"
                ],
            },
        }
        ...
        with patch(
            "concrete.db_decommission._initialize_github_client",
        ...
        assert (
            mock_github_client.create_or_update_file.call_count == 2
        )  # One for each file
