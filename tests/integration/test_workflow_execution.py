...
class TestWorkflowIntegration:
    ...
    def real_config_path(self, tmp_path):
        ...
        config = {
            "mcpServers": {
                ...
                "repomix": {"command": "npx", "args": ["repomix", "--mcp"], "env": {}},
            }
        }
    ...
    def mock_mcp_clients(self):
        ...
        with (
            patch("clients.RepomixMCPClient") as mock_repomix,
            ...
            mock_repomix_instance.pack_remote_repository = AsyncMock(
                return_value={
                    "success": True,
            ...
            mock_repomix_instance.pack_repository = AsyncMock(
                return_value={
                    "repository_url": "https://github.com/test/repo",
            ...
            mock_repomix_instance.call_tool_with_retry = AsyncMock(
                return_value={
                    "success": True,
                    "repository_url": "https://github.com/test/repo",
                    "files_packed": 156,
            ...
            mock_github_instance.analyze_repo_structure = AsyncMock(
                return_value={
                    "repository_url": "https://github.com/test/repo",
                    "owner": "test",
                    "repo": "repo",
            ...
            mock_github_instance.get_repository = AsyncMock(
                return_value={
                    "owner": "test",
                    "repo": "repo",
                    "full_name": "test/repo",
            ...
            mock_github_instance.create_pull_request = AsyncMock(
                return_value={
                    "success": True,
                    "number": 42,
            ...
            mock_github_instance.call_tool_with_retry = AsyncMock(
                return_value={
                    "success": True,
                    "repository_url": "https://github.com/test/repo",
                    "owner": "test",
            ...
            mock_slack_instance.list_channels = AsyncMock(
                return_value=[
                    {
                        "id": "C01234567",
                        "name": "general",
            ...
            mock_slack_instance.call_tool_with_retry = AsyncMock(
                return_value={
                    "success": True,
                    "channel": "C01234567",
                    "ts": "1234567890.123456",
    ...
    @pytest.mark.asyncio
    async def test_complete_repo_analysis_workflow(
        self, real_config_path, mock_mcp_clients
    ):
        ...
        with (
            patch(
                "clients.GitHubMCPClient",
                return_value=mock_mcp_clients["github_instance"],
            ),
            patch(
                "clients.RepomixMCPClient",
                return_value=mock_mcp_clients["repomix_instance"],
            ),
            patch(
                "clients.SlackMCPClient",
                return_value=mock_mcp_clients["slack_instance"],
            ),
        ):
            workflow = (
                WorkflowBuilder(
                ...
                .step(
                    ...
                    lambda context, step, **params: validate_repository_step(
                        context, step, **params
                    ),
                    parameters={
    ...
    @pytest.mark.asyncio
    async def test_database_decommission_workflow_simulation(
        self, real_config_path, mock_mcp_clients
    ):
            ...
            if step in completed_steps:
                ...
                if not step_result.get("error"):
                    assert (
                        ...
                        or "database_name" in step_result
                    )
    ...
    async def test_error_recovery_and_fallback_workflow(
        ...
        mock_mcp_clients[
            "repomix_instance"
        ].call_tool_with_retry.side_effect = Exception(
            "Repomix service temporarily unavailable"
        )
        ...
        workflow = (
            ...
            .step(
                ...
                lambda context, step, **params: merge_analysis_results(
                    context, step, **params
                ),
    ...
    async def test_parallel_execution_workflow(
        ...
        mock_mcp_clients["repomix_instance"].pack_repository = AsyncMock(
            side_effect=delayed_pack
        )
        mock_mcp_clients["github_instance"].analyze_repo_structure = AsyncMock(
            side_effect=delayed_analyze
        )
        ...
        parallel_workflow = (
            ...
            .step(
                ...
                lambda context, step, **params: self._consolidate_results_step(
                    context, step, **params
                ),
                depends_on=[
                    "pack_repo1",
    ...
    async def _consolidate_results_step(self, ctx, step):
        return {
            "total_repos": 2,
            "results_collected": len(
                [k for k in ctx._shared_context.keys() if "repo" in k]
            ),
        }
    ...
    async def test_workflow_context_sharing(self, real_config_path, mock_mcp_clients):
        ...
        async def context_consumer_step(context, step):
            ...
            return {
                ...
                "processed_items": len(
                    producer_data.get("data_payload", {}).get("items", [])
                ),
            }
        ...
        workflow = (
            ...
            .step(
                "producer",
                "Data Producer Step",
                lambda context, step, **params: context_producer_step(
                    context, step, **params
                ),
            )
            .step(
                "consumer",
                "Data Consumer Step",
                lambda context, step, **params: context_consumer_step(
                    context, step, **params
                ),
                depends_on=["producer"],
            )
            .repomix_pack_repo(
                "pack_repo", "https://github.com/test/repo", depends_on=["consumer"]
            )
            .step(
                "final_processor",
                "Final Processing Step",
                lambda context, step, **params: self._final_processor_step(
                    context, step, **params
                ),
                depends_on=["pack_repo"],
            )
