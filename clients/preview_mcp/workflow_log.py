...
class TableData:
    ...
    def to_markdown(self) -> str:
        ...
        for row in self.rows:
            ...
            padded_row = row[: len(self.headers)] + [""] * (
                len(self.headers) - len(row)
            )
...
class WorkflowLog:
    ...
    def append_sunburst(
        ...
        sunburst_data = SunburstData(
            labels=labels, parents=parents, values=values, title=title, colors=colors
        )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for entry_type in LogEntryType:
            entry_counts[entry_type.value] = len(
                [e for e in self.entries if e.entry_type == entry_type]
            )
        ...
        return {
            ...
            "last_updated": (
                self.entries[-1].timestamp.isoformat() if self.entries else None
            ),
        }
...
class WorkflowLogManager:
    ...
    def get_all_summaries(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            workflow_id: log.get_summary() for workflow_id, log in self.logs.items()
        }
...
def log_info(
    workflow_id: str, message: str, metadata: Optional[Dict[str, Any]] = None
) -> str:
    """Log an info message."""
