...
class WorkflowContext(BaseModel):
    ...
    def add_step(
        ...
        step = WorkflowStep(name=name, input_data=input_data or {})
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        failed_steps = sum(
            1 for step in self.steps if step.status == WorkflowStatus.FAILED
        )
...
class WorkflowResult(BaseModel):
    ...
    def from_context(cls, context: WorkflowContext) -> "WorkflowResult":
        ...
        completed_steps = sum(
            1 for step in context.steps if step.status == WorkflowStatus.COMPLETED
        )
