...
class SlackMCPClient(BaseMCPClient):
    ...
    async def post_message(
        ...
        try:
            ...
            if message_result["success"]:
                ...
                logger.warning(
                    f"Failed to post message: {result.get('error', 'Unknown error')}"
                )
            ...
            return {"success": False, "channel": channel_id, "error": str(e)}
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"SlackMCPClient ({self.server_name}) health check successful."
            )
            return True
        except Exception as e:
            logger.warning(
                f"SlackMCPClient ({self.server_name}) health check failed: {e}"
            )
            return False
    ...
    async def list_channels(
        ...
        try:
            ...
            if result.get("ok"):
                ...
                logger.warning(
                    f"Failed to list channels: {result.get('error', 'Unknown error')}"
                )
                return []
    ...
    async def add_reaction(
        ...
        try:
            ...
            if not reaction_result["success"]:
                ...
                logger.warning(
                    f"Failed to add reaction {name}: {reaction_result['error']}"
                )
            else:
    ...
    async def list_users(
        self, limit: int = 1000, include_locale: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if result.get("ok"):
                ...
                logger.warning(
                    f"Failed to list users: {result.get('error', 'Unknown error')}"
                )
                return []
    ...
    async def create_channel(
        ...
        try:
            ...
            if not create_result["success"]:
                ...
                logger.warning(
                    f"Failed to create channel {name}: {create_result['error']}"
                )
            else:
    ...
    async def invite_user_to_channel(self, channel: str, user: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not invite_result["success"]:
                ...
                logger.warning(
                    f"Failed to invite user {user} to {channel}: {invite_result['error']}"
                )
            else:
            ...
            return {"success": False, "channel": channel, "user": user, "error": str(e)}
