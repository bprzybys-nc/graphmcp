...
class RepomixMCPClient(BaseMCPClient):
    ...
    async def pack_remote_repository(
        ...
        """
        params = {"remote": repo_url}  # Changed 'url' to 'remote'
        ...
        try:
            logger.info(
                f"🔍 DEBUG: Calling pack_remote_repository with params: {params}"
            )
            ...
            ensure_serializable(pack_result)
            logger.info(
                f"Packed remote repository {repo_url}: {pack_result['files_packed']} files"
            )
            return pack_result

        except Exception as e:
            logger.error(f"Failed to pack remote repository {repo_url}: {e}")
            return {"success": False, "repository_url": repo_url, "error": str(e)}
