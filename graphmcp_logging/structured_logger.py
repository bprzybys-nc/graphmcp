...
class StructuredLogger:
    ...
    def _setup_console_handler(self) -> None:
        ...
        class EnhancedConsoleFormatter(logging.Formatter):
            ...
            def _format_summary_message(self, message: str, color: str) -> str:
                ...
                if (
                    "Environment validated:" in message
                    or "parameters loaded" in message
                ):
    ...
    def log_structured(self, entry: LogEntry) -> None:
        ...
        if self.config.output_format in [
            "json",
            "dual",
        ] and self.config.is_level_enabled(entry.level, "file"):
        ...
        if self.config.output_format in [
            "console",
            "dual",
        ] and self.config.is_level_enabled(entry.level, "console"):
    ...
    def log_progress(self, progress: ProgressEntry) -> None:
        ...
        if self.config.output_format in ["console", "dual"]:
            if progress.status == "started":
                ...
                progress.status == "progress" and progress.progress_percent is not None
            ):
                # Create visual progress bar
                progress_bar = self._create_progress_bar(progress.progress_percent)
                eta_text = (
                    f" ETA: {progress.eta_seconds:.0f}s" if progress.eta_seconds else ""
                )
    ...
    def log_section_separator(self, section_title: str) -> None:
        ...
        if self.config.output_format in [
            "console",
            "dual",
        ] and self.config.is_level_enabled("INFO", "console"):
            self._write_console_output(entry)
        ...
        if self.config.output_format in [
            "json",
            "dual",
        ] and self.config.is_level_enabled("INFO", "file"):
            self._write_json_output(entry)
