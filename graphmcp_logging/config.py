...
class LoggingConfig:
    ...
    def from_env(cls) -> "LoggingConfig":
        ...
        return cls(
            ...
            json_pretty_print=os.getenv("GRAPHMCP_JSON_PRETTY", "false").lower()
            == "true",
            structured_data_enabled=os.getenv(
                "GRAPHMCP_STRUCTURED_DATA", "true"
            ).lower()
            == "true",
            progress_tracking_enabled=os.getenv(
    ...
    def is_level_enabled(self, level: str, sink: Literal["console", "file"]) -> bool:
        ...
        return level_hierarchy.get(level.upper(), 0) >= level_hierarchy.get(
            threshold, 0
        )
    ...
    def validate(self) -> None:
        ...
        if self.console_level.upper() not in valid_levels:
            raise ValueError(
                f"Invalid console_level: {self.console_level}. Must be one of {valid_levels}"
            )

        if self.file_level.upper() not in valid_levels:
            raise ValueError(
                f"Invalid file_level: {self.file_level}. Must be one of {valid_levels}"
            )
