...
class ProgressTracker:
    ...
    def start_step(self, step_name: str, total_items: Optional[int] = None) -> str:
        ...
        progress_entry = ProgressEntry.create_started(
            workflow_id=self.workflow_id, step_name=step_name, total_items=total_items
        )
...
class WorkflowLogger:
    ...
    def log_environment_validation_summary(
        ...
        self.info(
            f"Environment validated: {total_params} parameters, {secrets_count} secrets, "
    ...
    def log_workflow_step_start(
        self, step_name: str, step_number: int, total_steps: int, description: str = ""
    ) -> None:
        """
    ...
    def log_workflow_step_tree(
        ...
        for i, sub_step in enumerate(sub_steps):
            ...
            if is_current:
                ...
                icon = (
                    "✅"
                    if sub_steps.index(sub_step)
                    < (sub_steps.index(current_sub_step) if current_sub_step else -1)
                    else "⏳"
                )
                prefix = "├─" if not is_last else "└─"
                tree_lines.append(f"{prefix} {icon} {sub_step}")
    ...
    def log_operation_duration(
        ...
        if items_processed:
            ...
            self.info(
                f"✅ {operation_name} completed: {items_processed} items in {duration_seconds:.1f}s "
                f"({rate:.1f} items/sec)"
            )
        else:
    ...
    def log_quality_assurance_summary(self, qa_results: List[Dict[str, Any]]) -> None:
        ...
        # Console summary
        passed_count = sum(
            1 for result in qa_results if result.get("status") == "passed"
        )
    ...
    def log_step_start(
        self, step_name: str, description: str = "", parameters: Optional[Dict] = None
    ) -> None:
    ...
    def log_step_complete(
        self, step_name: str, duration_ms: float, result: Optional[Dict] = None
    ) -> None:
        """Log workflow step completion."""
        self.info(
            f"Completed step: {step_name}", duration_ms=duration_ms, result=result
        )

    def log_step_error(
        self, step_name: str, error: str, context: Optional[Dict] = None
    ) -> None:
    ...
    def log_file_discovery(
        self, files: List[str], repo: str, pattern_matches: Optional[Dict] = None
    ) -> None:
        """Log file discovery with structured table data."""
        # Add section separator for repository processing
        self.structured_logger.log_section_separator(
        ...
        table_data = StructuredData.create_table(
            ...
            metadata={
                ...
                "total_matches": (
                    sum(pattern_matches.values()) if pattern_matches else 0
                ),
            },
        )
        self.structured_logger.log_structured_data(table_data)
    ...
    def log_pattern_discovery(
        self, patterns: Dict[str, List[str]], total_matches: int
    ) -> None:
    ...
    def print_file_hits_table(
        self, file_hits: List[Dict], title: str = "File Discovery"
    ) -> None:
        ...
        table_data = StructuredData.create_table(
            workflow_id=self.workflow_id, title=title, headers=headers, rows=rows
        )
        self.structured_logger.log_structured_data(table_data)

    def print_refactoring_groups(
        self, groups: List[Dict], title: str = "Refactoring Groups"
    ) -> None:
    ...
    def log_git_changes(
        self, changes: List[Dict[str, str]], title: str = "Git Changes Summary"
    ) -> None:
        """
    ...
    def log_workflow_metrics(self, metrics: Dict[str, Any]) -> None:
        ...
        metrics_data = StructuredData.create_metrics(
            workflow_id=self.workflow_id, title="Workflow Metrics", metrics=metrics
        )
        self.structured_logger.log_structured_data(metrics_data)
    ...
    def log_workflow_summary(self) -> None:
        ...
        summary_data = StructuredData.create_metrics(
            workflow_id=self.workflow_id, title="Workflow Summary", metrics=summary
        )
    ...
    def complete_progress(
        self, step_id: str, final_metrics: Optional[Dict] = None
    ) -> None:
    ...
    def log_workflow_end(
        ...
        **context,
        ...
        if step_name is None and result is None:
            ...
            status = "✅" if success else "❌"
            self.info(
                f"{status} Workflow step completed: {step_name}",
                result=result,
                success=success,
                **context,
            )
    ...
    def log_table(self, title: str, data, headers: list = None, **context) -> None:
        ...
        if isinstance(data, list) and len(data) > 0:
            if isinstance(data[0], dict):
                ...
                if headers is None:
                    headers = [
                        f"Column {i + 1}" for i in range(len(rows[0]) if rows else 0)
                    ]
        else:
    ...
    def log_step_end(
        self, step_name: str, result: dict, success: bool = True, **context
    ) -> None:
        """Backward compatibility for step end logging."""
        status = "✅" if success else "❌"
        self.info(
            f"{status} Completed step: {step_name}",
            result=result,
            success=success,
            **context,
        )
