...
async def validate_prp_requirements():
    ...
    builder.step_auto("test_step", "Test Step", test_function, parameters={"test": "value"})
    ...
    result = db_builder.with_repositories(["https://github.com/test/repo1", "https://github.com/test/repo2"])
    assert result is db_builder
    assert db_builder.target_repos == ["https://github.com/test/repo1", "https://github.com/test/repo2"]
    ...
    custom_workflow = new_builder.add_validation_step().add_repository_processing_step().build()
    ...
    comprehensive_workflow = (comprehensive_builder
                             .step_auto("comp_step1", "Comprehensive Step 1", mock_comprehensive_step)
                             .step_auto("comp_step2", "Comprehensive Step 2", mock_comprehensive_step, depends_on=["comp_step1"])
                             .build())
...
async def validate_performance_requirements():
    ...
    old_builder.step("old_step", "Old Step", lambda context, step, **params: test_func(context, step, **params))
    ...
    fluent_workflow = (DatabaseDecommissionWorkflowBuilder("fluent_test")
