...
async def mock_workflow_execution_test():
    ...
    async def validation_step(context, step, **params):
        return {"validation_passed": True, "database_name": params.get("database_name", "test_db")}
    ...
    builder = (builder
               .with_repositories(["https://github.com/test/integration-repo"])
               .with_slack_channel("integration-test-channel"))
    
    # Add steps using step_auto
    builder = (builder
               .step_auto("validate_environment", "Validation Step", validation_step)
               .step_auto("process_repositories", "Processing Step", processing_step, depends_on=["validate_environment"]))
...
async def test_fluent_interface_patterns():
    ...
    builder = (DatabaseDecommissionWorkflowBuilder("fluent_test_db")
               .with_repositories(["https://github.com/test/repo1", "https://github.com/test/repo2"])
...
async def test_parameter_management():
    ...
    builder = (DatabaseDecommissionWorkflowBuilder("param_test_db")
    ...
    assert repo_step.parameters["target_repos"] == ["https://github.com/testowner/testrepo"]
...
async def test_workflow_execution_integration():
    ...
    workflow = (builder
                .step_auto("step1", "First Step", step1)
